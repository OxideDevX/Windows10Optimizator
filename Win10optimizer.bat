...
ECHO ********** Disable tasks
REM  ********** Отключить ненужные задачи в планировщике заданий
...
REM  ********** Удалить телеметрию сбора данных и отключить голосовой помощник и ассистент Кортану
...
REM  ********** Отключить сбор данных(автоматическое логгирование)
...
ECHO ********** Disable Office telemetry(worked by installed office packet in you computer)
REM  ********** Отключить телеметрию MS Office(работает при наличии установленного офисного пакета)
...
ECHO ********** Disable Skype telemetry(don`t worked)
REM  ********** Отключить телеметрию Skype(может не функционировать)
...
ECHO ********** Disable and delete search and indexes in panel control
REM  ********** Отключить и удалить интернет-поиск и индексы в панеле управления
...
TASKKILL /f /im OneDrive.exe //ебнет к хуям процесс и иннцирует удаление всей такой хуйни
...
REM  ********** Очистить временные файлы и кэш иконок(миниатюр)
...
REM  ********** Очистить папки автозагрузки от мусора
...
:BLOCK
ECHO ********** Block hosts
REM  ********** Блокировать нежелательные веб узлы в файл hosts(перенаправит сетевую активность в 127.0.0.1). Сюда можно прописать адресса порносайтов и туда будет заблокирован доступ.
...
REM  ********** Блокировка портов (повышение безопасности). Блокируються самые расспространенные порты, путем их перенаправления на локалхост
